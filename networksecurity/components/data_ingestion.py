...
class DataIngestion:
    ...
    def export_collection_as_dataframe(self):
        """Export the collection from MongoDB as a dataframe"""
        try:
    ...
    def split_data_into_train_test(self,dataframe:pd.DataFrame):
        try:
            ...
            os.makedirs(dir_path,exist_ok=True)
            logging.info("Exporting the train and test data into the file")
            train_set.to_csv(self.data_ingestion_config.training_file_path,index=False,header=True)
            test_set.to_csv(self.data_ingestion_config.test_file_path,index=False,header=True)
            logging.info("Exited export_data_into_file method of Data_Ingestion class")
        except Exception as e:
            raise NetworkSecurityException(e,sys)
